...
import {
  ...
  Tag, ArrowRight, ArrowUp, ArrowDown
} from 'lucide-react';
import { useUIStore } from '../../../store/uiStore';
import { useCircuitStore } from '../../../store/circuitStore';
import { PinSide } from '../../../types/circuit';
...
// Main Component
// ============================================================
export function ICBuilderModal() {
    ...
    // Reset state when modal opens (transition from closed → open)
    const prevShowRef = useRef(false);
    useEffect(() => {
        const wasHidden = !prevShowRef.current;
        prevShowRef.current = showICBuilder;

        if (showICBuilder && wasHidden) {
            // Reset form state for a fresh session
    ...
    const togglePin = useCallback((nodeId: string, nodeType: string, nodeLabel: string) => {
        setPins(prev => {
            if (prev.some(p => p.nodeId === nodeId)) {
                ...
                // Reorder (simple reset for all sides)
                const sides: PinSide[] = ['left', 'right', 'top', 'bottom'];
                const result: PinDef[] = [];
                sides.forEach(s => {
                    const sPins = filtered.filter(p => p.side === s).sort((a, b) => a.order - b.order);
                    sPins.forEach((p, i) => result.push({...p, order: i}));
                });
                return result;
            } else {
                // Add pin
                const side: PinSide = nodeType === 'INPUT' ? 'left' : 'right';
    ...
    const switchPinSide = (nodeId: string) => {
        setPins(prev => {
            ...
            const pin = updated[idx];
            const sideOrder: PinSide[] = ['left', 'top', 'right', 'bottom'];
            const currentIdx = sideOrder.indexOf(pin.side);
            const nextSide = sideOrder[(currentIdx + 1) % sideOrder.length];

            const oldSide = pin.side;

            // Update the pin
            const newMaxOrder = Math.max(-1, ...updated.filter(p => p.side === nextSide).map(p => p.order));
            updated[idx] = {...pin, side: nextSide, order: newMaxOrder + 1};

            // Compact the old side's orders
            const remainingOnOldSide = updated.filter(p => p.side === oldSide).sort((a, b) => a.order - b.order);
            remainingOnOldSide.forEach((p, i) => {
                const globalIdx = updated.findIndex(u => u.nodeId === p.nodeId);
                updated[globalIdx].order = i;
            });
    ...
    const handleSave = useCallback(() => {
        ...
        const inputMarkers = pins
            .filter(p => terminalNodes.find((n: any) => n.id === p.nodeId)?.data.type === 'INPUT')
            .map(p => ({id: p.nodeId, label: p.label, side: p.side}));

        const outputMarkers = pins
            .filter(p => terminalNodes.find((n: any) => n.id === p.nodeId)?.data.type !== 'INPUT')
            .map(p => ({id: p.nodeId, label: p.label, side: p.side}));

        createIC(name, description, selectedNodeIds, inputMarkers, outputMarkers);
        setShowICBuilder(false);
    }, [name, description, selectedNodeIds, pins, createIC, setShowICBuilder, terminalNodes]);
    ...
    const topPins = pins.filter(p => p.side === 'top').sort((a, b) => a.order - b.order);
    const bottomPins = pins.filter(p => p.side === 'bottom').sort((a, b) => a.order - b.order);

    const icHeight = Math.max(100, Math.max(leftPins.length, rightPins.length) * 28 + 60);
    const icWidth = Math.max(140, Math.max(topPins.length, bottomPins.length) * 28 + 60);
            ...
            >
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[9px] text-dim font-bold uppercase tracking-[0.3em] mt-0.5 opacity-50">
                                {selectedNodes.length} components · {selectedEdges.length} wires
                                · {terminalNodes.length} terminals
                            </p>
                        </div>
                    </div>
                    <button onClick={() => setShowICBuilder(false)}
                            className="text-dim hover:text-main transition-colors p-2">
                        <X size={18}/>
                    </button>
                </div>
                ...
                <div className="flex-1 flex overflow-hidden min-h-0">
                    ...
                    <div className="flex-1 flex flex-col min-w-0 border-r border-border-main">
                        {/* Editor toolbar */}
                        <div
                            className="px-4 py-2 border-b border-border-main flex items-center justify-between shrink-0 bg-app">
                            <div className="flex items-center gap-2">
                <span className="text-[9px] font-black uppercase tracking-[0.3em] text-dim opacity-50">
                  Circuit Editor
                </span>
                                <span
                                    className="text-[9px] px-2 py-0.5 rounded-sm bg-main/5 border border-border-main text-dim font-bold">
                  Click terminals to define pins
                </span>
                            </div>
                            <div className="flex items-center gap-1">
                                <button onClick={() => setZoom(z => Math.min(4, z * 1.2))}
                                        className="p-1.5 text-dim hover:text-main transition-colors">
                                    <ZoomIn size={14}/>
                                </button>
                  ...
                  {Math.round(zoom * 100)}%
                                ...
                                <button onClick={fitView} className="p-1.5 text-dim hover:text-main transition-colors"
                                        title="Fit view">
                                    <Maximize2 size={14}/>
                                </button>
                            </div>
                        ...
                        >
                            ...
                            >
                                {/* Grid pattern */}
                                <defs>
                                    <pattern id="ic-grid" width={20 * zoom} height={20 * zoom}
                                ...
                                <g transform={`translate(${pan.x}, ${pan.y}) scale(${zoom})`}>
                                    ...
                                    {selectedNodes.map((n: any) => {
                                        ...
                                        return (
                                            ...
                                            >
                                                {/* Selection border for pinned terminals */}
                                                {pinned && (
                                                    <rect
                                                        x={x - 4}
                                                        y={y - 4}
                                                ...
                                                <rect
                                                    ...
                                                    height={NODE_H}
                                                    rx={8}
                                                    fill={pinned ? `${color}10` : 'var(--bg-panel)'}
                                                    stroke={pinned ? color : 'var(--border-main)'}
                                                ...
                                                {pinned && (
                                                    <g transform={`translate(${x + NODE_W - 18}, ${y + 4})`}>
                                                        ...
                                                        <text x={7} y={10.5} fill="white" fontSize={8} fontWeight={900}
                                                              textAnchor="middle">✓
                                                        </text>
                                                    </g>
                                                )}
                                                ...
                                                {isTerminal && !pinned && (
                                                    <g transform={`translate(${x + NODE_W - 18}, ${y + 4})`}
                                                       opacity={0.3}>
                                                        <rect width={14} height={14} rx={2} fill="none" stroke={color}
                            ...
                            <div className="absolute bottom-3 left-3 flex gap-3">
                                <div
                                    className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-app/80 backdrop-blur-sm border border-border-main">
                                    <div className="w-2 h-2 rounded-full bg-emerald-500"/>
                                    <span
                                        className="text-[8px] font-black uppercase tracking-widest text-dim">Input</span>
                                </div>
                                <div
                                    className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-app/80 backdrop-blur-sm border border-border-main">
                                    <div className="w-2 h-2 rounded-full bg-blue-500"/>
                                    <span
                                        className="text-[8px] font-black uppercase tracking-widest text-dim">Output</span>
                                </div>
                                <div
                                    className="flex items-center gap-1.5 px-2 py-1 rounded-sm bg-app/80 backdrop-blur-sm border border-border-main">
                                    <div className="w-2 h-2 rounded-full bg-gray-500"/>
                                    <span
                                        className="text-[8px] font-black uppercase tracking-widest text-dim">Logic</span>
                                </div>
                            </div>
                            ...
                            {pins.length === 0 && (
                                <div
                                    className="absolute top-3 left-1/2 -translate-x-1/2 px-4 py-2 rounded-sm bg-app/90 backdrop-blur-sm border border-border-main animate-pulse">
                  <span className="text-[9px] font-black uppercase tracking-widest text-dim">
                    Click on <span className="text-emerald-500">INPUT</span> or <span
                      className="text-blue-500">OUTPUT</span> nodes to define IC pins
                  </span>
                                </div>
                            )}
                        </div>
                    ...
                    <div className="w-[340px] shrink-0 flex flex-col overflow-y-auto bg-app">
                        <div className="p-5 space-y-5 flex-1">
                            ...
                            <div className="space-y-3">
                                <div>
                                    <label
                                        className="block text-[9px] font-black uppercase tracking-[0.4em] text-dim opacity-50 mb-2">IC
                                        Name</label>
                                    <input
                                        type="text"
                                        value={name}
                                ...
                                <div>
                                    <label
                                        className="block text-[9px] font-black uppercase tracking-[0.4em] text-dim opacity-50 mb-2">Subtitle</label>
                                    <input
                                        type="text"
                                        value={description}
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    <label
                                        className="text-[9px] font-black uppercase tracking-[0.4em] text-dim opacity-50">
                                ...
                                {pins.length === 0 ? (
                                    <div
                                        className="py-6 border border-dashed border-border-main rounded-sm text-center">
                                    ...
                                    <div className="space-y-1">
                                        <div className="space-y-4">
                                            ...
                                            {[
                                                {
                                                    side: 'left' as PinSide,
                                                    label: 'Left Pins',
                                                    color: '#10b981',
                                                    icon: <ArrowRight size={8}/>
                                                },
                                                {
                                                    side: 'top' as PinSide,
                                                    label: 'Top Pins',
                                                ...
                                                {
                                                    side: 'right' as PinSide,
                                                    label: 'Right Pins',
                                                    color: '#3b82f6',
                                                    icon: <ArrowRight size={8} className="rotate-180"/>
                                                },
                                                {
                                                    side: 'bottom' as PinSide,
                                                    label: 'Bottom Pins',
                                                    color: '#f59e0b',
                                                    icon: <ArrowUp size={8}/>
                                                },
                                            ].map(group => {
                                                const groupPins = pins.filter(p => p.side === group.side).sort((a, b) => a.order - b.order);
                                                if (groupPins.length === 0) return null;

                                                return (
                                                    <div key={group.side} className="space-y-1">
                                                        <div
                                                            className="text-[8px] font-black uppercase tracking-[0.3em] opacity-60 mb-1 flex items-center gap-2"
                                                            style={{color: group.color}}>
                                                            {group.icon}
                                                            {group.label} ({groupPins.length})
                                                        </div>
                                                        {groupPins.map(p => (
                                                            <PinRow
                                                                key={p.nodeId}
                                                                ...
                                                                isFirst={p.order === 0}
                                                                isLast={p.order === groupPins.length - 1}
                                                            />
                                                        ))}
                                                    </div>
                                                );
                            ...
                            {pins.length > 0 && (
                                <div>
                                    <label
                                        className="block text-[9px] font-black uppercase tracking-[0.4em] text-dim opacity-50 mb-3">
...
function ICPreview({
  ...
  return (
    <div className="relative flex flex-col items-center">
      {/* Top pins */}
      <div className="flex justify-evenly w-full px-10 mb-[-4px]">
        {topPins.map((p) => (
          <div key={p.nodeId} className="flex flex-col items-center gap-1">
             <span className="text-[7px] text-accent-purple font-black font-mono h-8 flex items-end rotate-180 [writing-mode:vertical-lr] opacity-70 uppercase">
              {p.label}
            </span>
            <div className="h-4 w-[1.5px] bg-accent-purple/40" />
            <div className="w-2 h-2 rounded-full border-[1.5px] border-accent-purple bg-app -mb-1 z-10" />
          </div>
        ))}
      </div>
      ...
      <div className="flex items-stretch">
        {/* Left pins */}
        <div className="flex flex-col justify-evenly pr-0 z-10" style={{ height: `${height}px` }}>
          {leftPins.map((p) => (
        ...
        {/* IC body */}
        <div
          className="relative border-2 border-main/25 rounded-md bg-app flex items-center justify-center shadow-lg"
          style={{ width: `${width}px`, height: `${height}px` }}
        >
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-3 rounded-b-full bg-main/5 border-x border-b border-border-main" />
          <div className="text-center px-4">
            <div className="text-[11px] font-black text-main uppercase tracking-widest leading-tight" style={{ wordBreak: 'break-word' }}>
              {name}
            </div>
            {description && (
              <div className="text-[8px] text-dim font-bold mt-1.5 uppercase tracking-wider opacity-40">
                {description}
              </div>
            )}
          </div>
        ...
        {/* Right pins */}
        <div className="flex flex-col justify-evenly pl-0 z-10" style={{ height: `${height}px` }}>
          {rightPins.map((p) => (
            <div key={p.nodeId} className="flex items-center gap-1">
      ...
      {/* Bottom pins */}
      <div className="flex justify-evenly w-full px-10 mt-[-4px]">
        {bottomPins.map((p) => (
          <div key={p.nodeId} className="flex flex-col items-center gap-1">
            <div className="w-2 h-2 rounded-full border-[1.5px] border-orange-500 bg-app -mt-1 z-10" />
            <div className="h-4 w-[1.5px] bg-orange-500/40" />
            <span className="text-[7px] text-orange-500 font-black font-mono h-8 [writing-mode:vertical-lr] opacity-70 uppercase">
              {p.label}
            </span>
          </div>
        ))}
